<template>
  <div id="mapChart" ref="mapChart" style="position: absolute;width:100%;height:100%"></div>
</template>
...
export default {
  ...
  created() {
    var url =
      "https://webapi.amap.com/maps?v=1.4.15&key=9463f69a47b7b1c9a0ef0602bf560e07&callback=onLoad";
  ...
  methods: {
    mapFn() {
      ...
      window.onLoad = function () {
        ...
        _this.map.clearMap();
        for (let val of _this.list) {
          new AMap.Marker({
            icon: val.citycode != undefined ? _this.style[0] : _this.style[1],
            map: _this.map,
            position: val.center,
          });
        }
